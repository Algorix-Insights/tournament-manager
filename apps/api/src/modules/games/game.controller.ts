...
export class GameController {
  ...
  static async create(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!nombre || !genero) {
        res.status(400).json({ error: 'Nombre y Género son obligatorios' });
        return;
      }

      const game = await GameService.create({ nombre, genero });
  ...
  static async update(req: Request, res: Response): Promise<void> {
    try {
      ...
      const updated = await GameService.update(id, req.body);
      res.json(updated);
    } catch (error: any) {
      if (error.code === 'P2025') {
        res.status(404).json({ error: 'Videojuego no encontrado' });
